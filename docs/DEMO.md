...
- 可先输入 `hel` 再按 `Tab`，展示 Shell 唯一前缀命令补全。
- 启动阶段会播放由视频前 4 秒转换的 16 色 VGA 动画，随后进入 32 位内核。
- 输入 `gui` 展示彩色桌面，按 `Q` 返回；输入 `game` 后用左右方向键接住星号。
- Shell 中用 `PageUp` 回看旧输出、`PageDown` 返回；长命令会自动跨行并显示硬件光标。
- GUI 中使用上下方向键选择区域，按 `Enter` 打开 Monitor、文件列表或游戏。
